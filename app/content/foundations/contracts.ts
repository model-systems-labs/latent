...
export const foundationExerciseContracts: readonly ExerciseContract[] = [
  ...
  define("arrays-and-shapes", "reshape-array", "Reshape an array", "reshape_array", [
    ...
    { id: "wrong-size", label: "Rejects a shape with the wrong value count", args: [[1, 2, 3], [2, 2]], assertions: [throwsWith("preserve-size", "Explain that reshape must preserve the number of values", "preserve")] },
  ]),
  define("vector-operations", "add-vectors", "Add vectors", "add_vectors", [
    ...
    { id: "length-mismatch", label: "Rejects vectors with different lengths", args: [[1, 2], [3]], assertions: [throwsWith("same-shape", "Require the same vector shape", "same shape")] },
  ]),
  ...
  define("dot-products", "cosine-similarity", "Cosine similarity", "cosine_similarity", [
    ...
    { id: "zero-vector", label: "Rejects a vector with no direction", args: [[0, 0], [1, 0]], assertions: [throwsWith("nonzero", "Require nonzero vectors", "nonzero")] },
  ]),
  define("matrix-multiplication", "matrix-vector-product", "Matrix-vector product", "matrix_vector_product", [
    ...
    { id: "inner-mismatch", label: "Rejects an incompatible vector width", args: [[[1, 2, 3]], [1, 2]], assertions: [throwsWith("inner-dimension", "Explain the matrix-vector shape mismatch", "columns")] },
  ]),
  define("matrix-multiplication", "linear-layer", "Linear layer", "linear_layer", [
    ...
    { id: "bias-mismatch", label: "Rejects the wrong bias width", args: [[1, 2], [[1, 0], [0, 1]], [0]], assertions: [throwsWith("bias-width", "Require one bias per output", "bias")] },
  ]),
  define("batches-and-broadcasting", "add-row-bias", "Add row bias", "add_row_bias", [
    ...
    { id: "bias-mismatch", label: "Rejects the wrong bias width", args: [[[1, 2]], [1]], assertions: [throwsWith("matching-width", "Require matching row and bias widths", "width")] },
  ]),
  define("batches-and-broadcasting", "batch-linear", "Batched linear layer", "batch_linear", [
    ...
    { id: "input-mismatch", label: "Rejects the wrong input width", args: [[[1, 2, 3]], [[1, 2]], [0]], assertions: [throwsWith("feature-width", "Require inputs to match the weight width", "width")] },
  ]),
  ...
  define("ml-training-data", "features-targets", "Features and targets", "features_and_targets", [
    ...
    { id: "ragged", label: "Rejects a ragged table", args: [[[1, 2, 0], [3, 1]]], assertions: [throwsWith("rectangular", "Require a rectangular table", "rectangular")] },
  ]),
  define("ml-training-data", "holdout-split", "Holdout split", "holdout_split", [
    ...
    { id: "duplicate-index", label: "Rejects duplicate validation indices", args: [[[1], [2]], [0, 1], [1, 1]], assertions: [throwsWith("unique-indices", "Require unique validation indices", "unique")] },
  ]),
  define("ml-linear-regression", "linear-prediction", "Linear prediction", "linear_prediction", [
    ...
    { id: "mismatch", label: "Rejects a missing weight", args: [[1, 2], [3], 0], assertions: [throwsWith("one-weight", "Require one weight per feature", "one weight per feature")] },
  ]),
  define("ml-linear-regression", "mean-squared-error", "Mean squared error", "mean_squared_error", [
    ...
    { id: "empty", label: "Rejects an empty batch", args: [[], []], assertions: [throwsWith("nonempty", "Require at least one example", "at least one")] },
  ]),
  ...
  define("ml-gradient-descent", "gradient-step", "Gradient step", "gradient_step", [
    ...
    { id: "negative-rate", label: "Rejects a negative learning rate", args: [1, 0, { weight: 1, bias: 1 }, -0.1], assertions: [throwsWith("nonnegative-rate", "Require a nonnegative learning rate", "learning rate")] },
  ]),
  ...
  define("ml-binary-classification", "binary-cross-entropy", "Binary cross-entropy", "binary_cross_entropy", [
    ...
    { id: "invalid-target", label: "Rejects a target outside zero or one", args: [[0.5], [2]], assertions: [throwsWith("binary-target", "Require binary targets", "0 or 1")] },
  ]),
  define("ml-neural-networks", "relu", "ReLU", "relu", [
    ...
    { id: "matrix", label: "Rejects a matrix input", args: [[[1, 2], [3, 4]]], assertions: [throwsWith("vector-only", "Require a vector", "vector")] },
  ]),
