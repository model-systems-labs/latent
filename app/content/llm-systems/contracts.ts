...
export const llmSystemsExerciseContracts: readonly ExerciseContract[] = [
  ...
  defineExerciseContract({
    ...
    cases: [
      {
        id: "large-logits",
        label: "Normalizes large logits without overflow",
        args: [[1001, 1000, 999]],
        assertions: [
          length("vocabulary-width", "Returns one probability per logit", 3),
          finite("first-finite", "Keeps the first probability finite", [0]),
          finite("second-finite", "Keeps the second probability finite", [1]),
          finite("third-finite", "Keeps the third probability finite", [2]),
          range("first-probability", "Normalizes the first probability", 0.66524, 0.66525, [0]),
          range("second-probability", "Normalizes the second probability", 0.24472, 0.24474, [1]),
          range("third-probability", "Normalizes the third probability", 0.09002, 0.09004, [2]),
        ],
      },
    ],
  }),
    ...
    cases: [
      {
        ...
        args: [[0, 1], [[2, 0], [0, 4]]],
        assertions: [equal("averaged-vector", "Returns the coordinate-wise context average", [1, 2])],
      },
    ],
  }),
  defineExerciseContract({
    ...
    cases: [
      {
        ...
        assertions: [range("certain-loss", "Produces low loss for the certain target", 0.1, 0.11)],
      },
      {
        id: "uncertain-target",
        label: "Scores a low-probability target",
        args: [[0.45, 0.1, 0.45], 1],
        assertions: [range("uncertain-loss", "Produces higher loss for the uncertain target", 2.3, 2.31)],
      },
    ],
  }),
  defineExerciseContract({
