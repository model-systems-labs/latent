import { createInvocationGuard } from "./invocation-contract.mjs";
...
safeAddEventListener("message", async (event) => {
  ...
  try {
    const guard = createInvocationGuard(args ?? []);
    const callable = loadEntrypoint(String(source ?? ""), entrypoint);
    if (typeof callable !== "function") {
      throw new Error(`Define function ${entrypoint.functionName}.`);
    }
    const value = await callable(...guard.args);
    safePostMessage({ nonce, ok: true, value, purity: guard.assess(value) });
  } catch (error) {
    safePostMessage({
      nonce,
      ok: false,
