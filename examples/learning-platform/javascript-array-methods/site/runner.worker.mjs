...
safeAddEventListener("message", async (event) => {
  ...
  try {
    ...
    for (const exerciseCase of cases ?? []) {
      const value = await executeCase(source, entrypoint, exerciseCase, limits);
      if (serializedOutputBytes(value) > limits.maxOutputBytes) {
        throw new Error("The serialized result exceeded the exercise output limit.");
      }
      results.push(assessCase(exerciseCase, value));
    }
