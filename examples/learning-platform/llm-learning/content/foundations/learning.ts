...
export const foundationLearningOutcomes = {
  ...
  "batches-and-broadcasting": {
    concept: "A bias sets one learned baseline per output coordinate, and broadcasting adds those same baselines to every row processed by the shared layer.",
    before: "The bias appears to be an unexplained extra vector added after matrix multiplication.",
    after: "You can distinguish what weights and bias do, then explain why one bias vector is shared across the entire batch.",
    check: {
      id: "shared-bias-purpose",
      prompt: "Why does a batched linear layer add the same bias vector to every output row?",
      choices: [
        { id: "shared-layer", label: "Each row uses the same learned output baselines" },
        { id: "row-parameters", label: "Each row needs separately learned parameters" },
        { id: "batch-size", label: "Bias changes the number of rows" },
      ],
      correctChoiceId: "shared-layer",
      explanation: "The bias belongs to the layer's output coordinates, not to an individual example. Every row uses the same layer, so every row receives the same learned output baselines.",
    },
  },
  "ml-training-data": {
