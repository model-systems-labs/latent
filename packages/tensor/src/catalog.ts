import { LATENT_TENSOR_PATH } from "./browser-source.js";

export type LatentTensorOperationCategory =
  | "creation"
  | "shape"
  | "math"
  | "reduction"
...
export type LatentTensorOperation = {
  name: string;
  category: LatentTensorOperationCategory;
  purpose: string;
};

export const LATENT_TENSOR_OPERATIONS = [
...
] as const satisfies readonly LatentTensorOperation[];

export type LatentTensorOperationName = typeof LATENT_TENSOR_OPERATIONS[number]["name"];

export function latentTensorImport(operations: readonly string[]): string {
  if (operations.length === 0) return "";
  return `import { ${operations.join(", ")} } from "../${LATENT_TENSOR_PATH}";`;
}

export function latentTensorOperations(names: readonly string[]): LatentTensorOperation[] {
  const byName = new Map<string, LatentTensorOperation>(
    LATENT_TENSOR_OPERATIONS.map((operation) => [operation.name, operation]),
  );
  return names
    .map((name) => byName.get(name))
    .filter((operation): operation is LatentTensorOperation => Boolean(operation));
}
