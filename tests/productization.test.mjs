...
before(async () => {
  ...
  [course, learning, portfolio, projectWorkspace, learnerState, canonicalProject, fileStatus, contracts, template, provenance] = await Promise.all([
    ...
    vite.ssrLoadModule("/examples/learning-platform/llm-learning/content/llm-systems/learning.ts"),
    ...
    vite.ssrLoadModule("/examples/learning-platform/llm-learning/lessons/provenance.ts"),
  ]);
});
...
test("reviewed lessons do not reintroduce the two remediated source patterns", async () => {
  ...
  const rnnTrainerSources = await Promise.all([
    "examples/learning-platform/llm-learning/lessons/model/character-rnn-training.ts",
  ...
  const bpeSources = await Promise.all([
    "examples/learning-platform/llm-learning/lessons/model/subword-tokenization.ts",
    "examples/learning-platform/llm-learning/lessons/lesson-flair.ts",
    "examples/learning-platform/llm-learning/lessons/exercise-contracts.ts",
    "examples/learning-platform/llm-learning/content/llm-systems/contracts.ts",
  ].map((path) => readFile(new URL(path, root), "utf8")));
